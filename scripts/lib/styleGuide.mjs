export const BRAND = {
  colors:
    'estrictamente rojo vibrante (#E2231A aprox.) y blanco; el rojo domina fondos/acentos y el blanco se usa para espacios negativos y tipografía (o viceversa); usar negro solo si es indispensable para la legibilidad del texto, sin introducir ningún otro color',
  style:
    'ilustración 2D animada, estilo flat/vector moderno, formas limpias y geométricas, leves líneas de movimiento o destellos que sugieren energía y tecnología, estética premium de canal tech',
  mascot:
    'mantener un motivo visual recurrente (ej. un ícono de cerebro-circuito o un pequeño bot/robot estilizado) presente en todas las slides para reforzar la identidad de marca',
  typography:
  ...
  channel: 'canal de Instagram de contenido sobre Inteligencia Artificial',
};
...
function roleInstructions(role, slideNumber, totalSlides) {
  switch (role) {
    ...
    case 'cta':
      return `Esta es la slide FINAL (CTA, ${slideNumber} de ${totalSlides}) del carrusel. Debe mostrar un llamado a la acción claro (ej. "Síguenos para más contenido de IA", "Comenta tu opinión", "Guarda este post"), con un diseño tipo botón o banner destacado, manteniendo exactamente el mismo estilo visual que las slides anteriores.`;
    default:
